...
import { filter } from "./Filter.functions";
...
export class Filter extends MainComponent {
  ...
  init() {
    ...
    const displayObj = this.$root.querySelector('[data-show]')
    $(displayObj).html(`Показать ${this.DATA.length} объектов`)
  ...
  onChange(event) {
    ...
    if (event.target.id === 'filter__dropdown') {
        filter(this.DATA, event.target.value, 'select')
    }
    else if (event.target.closest('#rooms')) {
        filter(this.DATA, event.target.value, 'rooms')
    }
